...
- `cn_im_hub.send_message`
  - 参数：`channel`、`target`、`message`、`camera_entity`、`use_selected_target`、`wechat_account_id`（可选）
...
- 个人微信在使用 `camera_entity` 发图时也沿用同样的多账号路由逻辑：优先按 `wechat_account_id`，否则按 `target` / `use_selected_target` 自动选择账号
